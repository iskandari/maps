...
var glslGeoProjection = require('glsl-geo-projection');
...
var Mapbox = function Mapbox(_ref) {
  ...
  var onViewStateChangeHandlerRef = React.useRef(null); // Initialize the map instance

  var initializeMap = React.useCallback(function (node) {
      ...
      try {
        ...
        if (missingExtensions.length > 0) {
          ...
          reglRef.current = webgl2Compat.overrideContextType(function () {
            return _regl__default['default']({
              container: node,
              extensions: requiredExtensions
            });
          });
        } else {
          reglRef.current = _regl__default['default']({
            container: node,
            extensions: requiredExtensions
          });
        }
...
function CircleRenderer(_ref) {
  ...
  function setCircle() {
    ...
    var handleXY = function () {
      ...
      var inter = turf.lineIntersect(line, circle); // but prefer rendering using intersection with circle to handle distortions near poles
...
function getInitialCenter(map, center) {
  if (Array.isArray(center) && center.length === 2 && isValidCoordinate(center[0], center[1])) {
    return new mapboxgl__default['default'].LngLat(center[0], center[1]);
  } else {
    if (center) {
...
function RegionPicker(_ref) {
  ...
  var initialCenter = React.useRef(getInitialCenter(map, initialCenterProp));
...
var useControls = function useControls() {
  ...
  var _useState = React.useState(map.getZoom()),
      zoom = _useState[0],
      setZoom = _useState[1];

  var _useState2 = React.useState(map.getCenter()),
      center = _useState2[0],
  ...
  try {
    ...
    var _temp4 = _switch(version, [[function () {
      ...
      return Promise.resolve(new Promise(function (resolve) {
        return zarr__default['default'](window.fetch, version).openGroup(source, function (err, l, m) {
          loaders = l;
          metadata = m;
          resolve();
        });
      ...
      return Promise.resolve(fetch(source + "/zarr.json").then(function (res) {
        ...
        return Promise.resolve(fetch(source + "/" + levels[0] + "/" + variable + "/zarr.json").then(function (res) {
          ...
          return Promise.resolve(Promise.all([].concat(levels.map(function (level) {
            return new Promise(function (resolve) {
              zarr__default['default'](window.fetch, version).open(source + "/" + level + "/" + variable, function (err, get) {
            ...
            return new Promise(function (resolve) {
              zarr__default['default'](window.fetch, version).open(source + "/" + levels[0] + "/" + key, function (err, get) {
...
function useRuler(_ref) {
  ...
  React.useEffect(function () {
    ...
    function addRuler() {
      ...
      rulerContainer = d3Selection.select(mapContainer).append('svg').classed('ruler', true).attr('width', width).attr('height', height).style('position', 'absolute').style('top', 0).style('left', 0).style('pointer-events', 'none'); // x-axis
